function findHeelEvents_openpose(output_name)
clearvars -except output_name
global cd name plot_dist plot_heel_down events_openpose time heel toe heel_data toe_data dist g check_events uiexit file
...
[pks_heel_down,locs_heel_down] = findpeaks(-dist);

events_openpose.heel_down_frames = locs_heel_down';
%%
...
plot_heel_down = plot(time(events_openpose.heel_down_frames),dist(events_openpose.heel_down_frames),'or');
xlabel('time (s)'),ylabel('vertical position of heel (pixels)')
title(name),legend('distance','Heel down','location','northeast');
...
uicontrol(gcf,'style','pushbutton','String','Create Down',...
   'units','normalized','Position',[.75 .55 0.2 0.05],'Callback',@downCreateHere); % pushbutton to heel down events
...
global plot_dist plot_heel_down events_openpose time dist

if ~isempty(get(plot_heel_down,'BrushData')) % check that user has brushed data
heeldownBrush = logical(get(plot_heel_down,'BrushData')); 
if true(logical(sum(heeldownBrush))) % check if user has brushed heel events
events_openpose.heel_down_frames(heeldownBrush) = []; % delete brushed heel events
end

delete(plot_dist);delete(plot_heel_down);% delete plot of events
plot_dist = plot(time,dist,'-k');
plot_heel_down = plot(time(events_openpose.heel_down_frames),dist(events_openpose.heel_down_frames),'or');
legend('distance','Heel down','location','northeast');
end
end
...
events_openpose.heel_down_frames(events_openpose.heel_down_frames == downNew(v)) = [];
end

events_openpose.heel_down_frames = sort([events_openpose.heel_down_frames downNew]); % add new events to vector with events
