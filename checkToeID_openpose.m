function checkToeID_openpose(output_name)
...
p = plot(squeeze(corrected_data.y(:,[heel toe],1)),'.-'); zoom on; grid on
legend('heel','toe','location','northwest'); xlabel('frames'),ylabel('vertical position (pixel)');title(name)
...
uicontrol(check_fig,'style','pushbutton','String','Clear toe ID',...
   'units','normalized','Position',[0.85 0.15 0.1 0.05],'Callback',@clearToe); % pushbutton to clear toe ID

uicontrol(check_fig,'style','pushbutton','String','Restore toe ID',...
   'units','normalized','Position',[0.85 0.1 0.1 0.05],'Callback',@restoreToe); % pushbutton to restore toe ID
...
p = plot(squeeze(corrected_data.y(:,[heel toe],1)),'.-'); zoom on; grid on; datacursormode on;
legend('heel','toe','location','northwest'); xlabel('frames'),ylabel('vertical position (pixel)');title(name)
set(gca,'xtick',frames)
...
%% "Clear toe" pushbutton
function clearToe(source,event)
global frame corrected_data p name frames frameInfo cursor_obj heel toe
if ~isempty(getCursorInfo(cursor_obj))
frame = getCursorInfo(cursor_obj);
frame = frame.Position(1);
corrected_data.x(frame,toe,:) = nan;
corrected_data.y(frame,toe,:) = nan;
subplot(3,3,[1 2 4 5 7 8]),delete(p)
p = plot(squeeze(corrected_data.y(:,[heel toe],1)),'.-'); zoom on; grid on; datacursormode on;
legend('heel','toe','location','northwest'); xlabel('frames'),ylabel('vertical position (pixel)');title(name)
set(gca,'xtick',frames)
...
%% "Restore toe" pushbutton
function restoreToe(source,event)
global frame corrected_data raw_data p name frames frameInfo cursor_obj heel toe
if ~isempty(getCursorInfo(cursor_obj))
frame = getCursorInfo(cursor_obj);
frame = frame.Position(1);
corrected_data.x(frame,toe,:) = raw_data(frame,toe,:);
corrected_data.y(frame,toe,:) = raw_data(frame,toe,:);
subplot(3,3,[1 2 4 5 7 8]),delete(p)
p = plot(squeeze(corrected_data.y(:,[heel toe],1)),'.-'); zoom on; grid on; datacursormode on;
legend('heel','toe','location','northwest'); xlabel('frames'),ylabel('vertical position (pixel)');title(name)
set(gca,'xtick',frames)
