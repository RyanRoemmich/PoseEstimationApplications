function gapFillHand_filter_openpose(output_name)
...
hand_markers = {'Wrist','Thumb1','Thumb2','Thumb3','Thumb4','Index1','Index2','Index3','Index4','Middle1','Middle2','Middle3','Middle4','Ring1','Ring2','Ring3','Ring4','Pinky1','Pinky2','Pinky3','Pinky4'};
axes = {'X','Y'};
...
data_openpose.gapFill_data.x = nan(size(data_openpose.corrected_data.x));
data_openpose.gapFill_data.y = nan(size(data_openpose.corrected_data.y));
frameInfo.isGapFilled.x = false(noFiles,noHandLandmarks); % logical matrix showing which frames are gap filled
frameInfo.isGapFilled.y = false(noFiles,noHandLandmarks); % logical matrix showing which frames are gap filled

for j = 1:noHandLandmarks
    if sum(~isnan(data_openpose.corrected_data.x(:,j))) > 1 % check that number of tracked data points exceed 1, otherwise "interp1gap" cannot gap fill
        data_openpose.gapFill_data.x(:,j) = interp1gap(data_openpose.time,data_openpose.corrected_data.x(:,j),data_openpose.time,0.120); % gap fill over gaps spanning less than 120 ms (ie. only gap filled over gaps of 2 or 1 frames at 25 Hz)
    end
    nan_data = isnan(data_openpose.corrected_data.x(:,j)); %check which frames are nan
    frameInfo.isGapFilled.x(nan_data,j) = ~isnan(data_openpose.gapFill_data.x(nan_data,j)); % check if nan frames have been interpolated

    if sum(~isnan(data_openpose.corrected_data.y(:,j))) > 1 % check that number of tracked data points exceed 1, otherwise "interp1gap" cannot gap fill
        data_openpose.gapFill_data.y(:,j) = interp1gap(data_openpose.time,data_openpose.corrected_data.y(:,j),data_openpose.time,0.120); % gap fill over gaps spanning less than 120 ms (ie. only gap filled over gaps of 2 or 1 frames at 25 Hz)
    end
    nan_data = isnan(data_openpose.corrected_data.y(:,j)); %check which frames are nan
    frameInfo.isGapFilled.y(nan_data,j) = ~isnan(data_openpose.gapFill_data.y(nan_data,j)); % check if nan frames have been interpolated
end; clearvars j
%% low-pass filter interpolated data
%% apply zero-lag low-pass Butterworth filtering with cut-off frequency of X Hz
data_openpose.filt_data.x = nan(size(data_openpose.gapFill_data.x));
data_openpose.filt_data.y = nan(size(data_openpose.gapFill_data.y));
ANyq = videoInfo.vid_openpose.FrameRate/2;
cf = 5; % set cut-off frequency
Wn = cf/ANyq/.802;
[B,A] = butter(2,Wn,'low');
for j = 1:noHandLandmarks
    data_openpose.filt_data.x(:,j) = nanfiltfilt(B,A,data_openpose.gapFill_data.x(:,j));
    data_openpose.filt_data.y(:,j) = nanfiltfilt(B,A,data_openpose.gapFill_data.y(:,j));
end; clearvars j
